...
EFI_LEGACY_IRQ_PRIORITY_TABLE_ENTRY *IrqPriorityTable[MAX_IRQ_PRIORITY_ENTRIES] = NULL;

//
// Legacy
//
EFI_LEGACY_IRQ_PRIORITY_TABLE_ENTRY IrqPriorityTableLeg[MAX_IRQ_PRIORITY_ENTRIES] = {
  {0x0B,0},
  {0x09,0},
  {0x0A,0},
  {0x05,0},
...
// PCH (Skylake+)
//
EFI_LEGACY_IRQ_PRIORITY_TABLE_ENTRY IrqPriorityTablePch[MAX_IRQ_PRIORITY_ENTRIES] = {
  {11,0},
  {10,0},
  {0,0},
  {0,0},
...
/**
  Detects chipset and sets PIRQ routing
  ...
  )
{
  ...
  switch (mDeviceId) {
    ...
    case 0x9b71:  // 9th Generation (Cometlake-U) DRAM Controller
      ...
      IrqPriorityTable = IrqPriorityTablePch;
      break;

    default:
      IrqPriorityTable = IrqPriorityTableLeg;
      break;
  }
}
